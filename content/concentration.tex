...
我们称$m_X$为随机变量$X$的\textbf{中位数}, 如果有$\P[X \geq m_X] \geq \frac12$, $\P[X \leq m_X] \leq \frac12$. 
注意中位数并不唯一: 例如对于等可能的Bernoulli分布$X \sim B(1, \frac12)$, 一切$m_X \in [0,1]$都是中位数. 
下面的命题说明关于均值的集中不等式和关于中位数的均值不等式之间可以相互转化. %HDS:exercise2.14
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\node at (0,0) {\large $\mathcal{C}$};
		
		\node at (1.5,0) {\large $+$};
		%--shape D
		\draw[thick] (2.4,0) circle (0.4);
		\node at (2.4,0) {\large $\mathcal{D}$};
		...
		\node at (5.2,0) {\large $\mathcal{C}+\mathcal{D}$};
	\end{tikzpicture}
...
\end{equation}
这样的形式可以推广至一般的泛函情形.
\begin{theorem}[Prékopa-Leindler不等式]\label{thm:Prékopa-LeindlerIneq}
...
%	我们考虑$\mathbb{S}^{n-1}$上的测地度量$\rho(\bm{x}, \bm{y}) = \arccos \langle \bm{x}, \bm{y} \rangle$, $\P$为$\mathbb{S}^{n-1}$上的均匀分布. 
%	记$H = \{\bm{x} \in \mathbb{S}^{n-1} \colon \rho(\bm{x}, \bm{y}) \leq \pi/2\}$为以$\bm{y}$为顶点的半球面, 显然有
%	$\alpha_{\mathbb{S}^{n-1}}(\epsilon) \geq 1 - \P[H_{\epsilon}]$. 
%	事实上, 球面上也有等周不等式: 球面$\mathbb{S}^{n-1}$上给定面积的子集中, 
%	$\epsilon$-扩张的面积最小的是球冠$H = \{\bm{x} \in \mathbb{S}^{n-1} \colon \rho(\bm{x}, \bm{y}) \leq c\}$, 其中$c$为某个常数. 
%	当我们取定$c = \pi/2$时, 对应的球冠是半球面, 从而集中度函数恰为$\alpha_{\mathbb{S}^{n-1}}(\epsilon) = 1 - \P[H_{\epsilon}]$.
%\begin{figure}[H]
%	\centering
%	\begin{tikzpicture}
%	% Define points
%    \coordinate (O) at (0,0); 		% Origin
%    \coordinate (P) at (0,.97);		% Point P
%    \coordinate (Q) at (1.75,.97);	% Point Q
%    \coordinate (R) at (2,0);		% Point R
%    \coordinate (S) at (-1.75,.97);	% Point S, Q的对称点
%    
%    % Draw the circles and hemisphere
%    \draw[thick] (O) circle (2); 		% S^{n-1}
%    \draw[thick,dashed] (P) circle (1.75); 	% comparison sphere
%    \fill[cyan] (180:2) arc (180:360:2cm and 2cm) -- cycle;
%    
%    % Annotation
%	\draw (O) -- (Q) -- (R) -- cycle;
%	\draw (O) -- (P);
%	\draw (Q) -- (S);
%	\markangle{Q}{O}{R}{$\theta$}{5}
%	\markangle{O}{Q}{P}{$\theta$}{5}
%	\node at (0,-1) {$H$};
%	\node at (0,1.35) {$\overline{H_{\epsilon}}$};
%	\node at (1.8,.4) {$\epsilon$};
%	\node at (1,-.25) {$1$};
%	\node at (-.3,.5) {$h$};
%	\node at (.95,1.2) {$r$};
%
%	\end{tikzpicture}
%	\caption{$1-\P[H_{\epsilon}]$的估计.}
%\end{figure}
%	\vspace{-.5cm}
%	\noindent
%	由几何知识我们有$\epsilon = 2 \sin(\frac{\theta}{2})$, 于是两个圆心间的距离为$h = \sin \theta = \epsilon \cos(\frac{\theta}{2}) \geq \sqrt{2} \epsilon$, 这是由于$\theta \in (0, \frac{\pi}{2})$
%	
%	这时$\overline{H_{\epsilon}}$的测度总是不大于标准化后虚线球面面积的一半, 于是
%	\begin{equation*}
%		\P[\overline{H_{\epsilon}}]
%		\leq r^{n-1}
%		= (1-(\sin \theta)^2)^{\frac{n-1}{2}}
%		\leq \exp \left( \frac{- (n-1)(\sin \theta)^2}{2} \right)
%	\end{equation*}
\end{example}
...
我们称函数$\psi \colon \R^n \to \R$是\textbf{$\gamma$-强凸}的, 如果存在某个$\gamma > 0$使得对任意$\lambda \in [0,1]$, $\bm{x}, \bm{y} \in \R^n$都有
\begin{equation*}
...
\begin{proof}[\keben 定理\ref{thm:Bobkov-Götze}的证明]%From APC550
	由定义, 命题(a)等价于
	\begin{equation*}
