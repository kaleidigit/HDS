...
若中心化随机变量$X - \mathbb{E}[X]$在$0$的某个邻域$I$内有\textbf{矩母函数}, 即在$\lambda \in I$上有$\varphi_X(\lambda) := \mathbb{E}[e^{\lambda (X - \mathbb{E}X)}] < \infty$, 那么$\lambda \in I \cap [0, \infty)$时, 取$f(u) = e^{\lambda u}$可以得到下述的\textbf{Chernoff不等式}: 
\begin{equation*}
...
\begin{theorem}[次高斯随机变量的等价定义]
	...
	\begin{enumerate}[label=(\Roman*)]
		...
		\item 存在常数$c_1, c_2 > 0$, 使得
			\begin{equation*}
				\P[|X| \geq t] \leq c_1 e^{-c_2 t},\quad \forall t > 0. 
			\end{equation*}
...
\begin{theorem}[Azuma–Hoeffding不等式]\label{thm:Azuma–Hoeffding}
	若鞅差序列$\{(D_k, \cF_k)\}_{k=1}^{\infty}$满足$D_k \in [a_k, b_k]$ a.s., 则$\sum_k D_k$是$\frac{1}{2}\sqrt{\sum_k L_k^2}$-次高斯的, 且有集中不等式
	\begin{equation*}
...
\begin{corollary}[有界差不等式]\label{cor:BddDiffIneq}
	设函数$f \colon \R^n \to \R$满足参数为$(L_1, \dots, L_n)$的有界差不等式, 随机向量$\bm{X} = (X_1, \dots, X_n)$各分量独立, 那么$f(\bm X)$是$\frac{1}{2} \sqrt{\sum_k L_k^2}$-次高斯的, 从而有集中不等式
	\begin{equation*}
...
\begin{proof}
	...
	\begin{equation*}
		B_k - A_k
		= \sup_{x, y} \mathbb{E}[f(\bm{X}_1^k, x, \bm{X}_{k+1}^n) - f(\bm{X}_1^k, y, \bm{X}_{k+1}^n) | X_1^{k-1}] 
		\leq L_k. 
	\end{equation*}
