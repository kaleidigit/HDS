...
\begin{equation}\label{eq:Markov'sInequality}
	\mathbb{P}[X \geq t] \leq \frac{\E f(X)}{f(t)}.
\end{equation}
特别地, 取$f(u) = u^2$, $X = |Y - \E Y|$, 有\emph{Chebyshev不等式}: 
\begin{equation*}
	\mathbb{P}[|Y - \E Y| \geq t] 
	\leq \frac{\var Y}{t^2}.
\end{equation*}
...
\begin{itemize}
	...
	\item 特别地, 取$\Phi(u) = u^2$时, 此时熵$\cH_\Phi(X) = \E X^2 - (\E X)^2$就是方差. 根据Chebyshev不等式, 我们可以利用方差熵来控制尾部概率; 
...
\begin{equation}\label{eq:EntropyByMGF}
	\cH(e^{\lambda X}) 
	= \lambda \E[ X e^{\lambda X}] - \E[e^{\lambda X}] \log \E[e^{\lambda X}]
	= \lambda \varphi_X'(\lambda) - \varphi_X(\lambda) \log \varphi_X(\lambda), 
\end{equation}
其中$\varphi_X(\lambda) = \E[e^{\lambda X}]$为矩母函数. 
...
\begin{lemma}[熵的张量化]
	...
	\begin{equation*}
		...
		\leq \E \left[ \sum_{k=1}^n \cH \left(e^{\lambda f_k(X_k)} \big| \bm X_k \right) \right],\; 
		\forall \lambda > 0. 
	\end{equation*}
\end{lemma}
\begin{proof}
...
\begin{theorem}
	...
	\begin{equation*}
		\mathbb{P}[f(X) \geq \E f(X) + \epsilon] \leq \beta(\epsilon),\; \forall \epsilon \geq 0, 
	\end{equation*}
	那么$\alpha_{\mathbb{P}}(\epsilon) \leq \beta(\epsilon / 2)$. 
\end{theorem}
\begin{proof}
	对任意$\mathcal{X}$中满足$\mathbb{P}[A] \geq 1/2$的可测集$A$, 构造$f_A(x) := \rho(x, A) \wedge \epsilon$. 
	注意到在$A$上有$f_A = 0$, 在$A$外有$f_A \leq \epsilon$, 所以$\E f_A(X) \leq \epsilon (1 - \mathbb{P}[A]) \leq \epsilon / 2$. 
	于是我们有
	\begin{equation*}
		1 - \mathbb{P}[A^\epsilon]
		= \mathbb{P}[X \in \bar A^{\epsilon}]
		= \mathbb{P}[f_A(X) \geq \epsilon] 
		\leq \mathbb{P}\left[f_A(X) \geq \E f_A(X) + \frac{\epsilon}{2}\right]
...
\begin{equation*}
	...
	:= \sup_{\|f\|_{Lip} \leq 1} \left[ \E_{\mathbb{Q}} f - \E_{\mathbb{P}} f \right]
...
\begin{equation*}
	\inf_{\M \in \mathbb{P}i(\mathbb{Q}, \mathbb{P})} \E_{\M} \left[ \rho(X, X') \right]
...
\begin{equation}
	D(\mathbb{Q} \| \mathbb{P})
	:= \E_{\mathbb{Q}} \left[ \log \frac{\dd \mathbb{Q}}{\dd \mathbb{P}} \right]
