...
\begin{itemize}
	...
	\item 此外, 取$\Phi(u) = - \log u$, 随机变量$Z = e^{\lambda X}$时, 
		...
		\end{equation*}
		这样的熵对应的是中心化的矩生成函数, 可以用来计算尾部概率的Chernoff界. 
\end{itemize}
...
\begin{example}[Gauss随机变量的熵]
	对于$X \sim \cN(\mu, \sigma^2)$, 其矩生成函数为$M_X(\lambda) = e^{\lambda^2 \sigma^2 / 2 + \lambda \mu}$, 于是
...
\begin{theorem}[Herbst方法]\label{thm:HerbstArgument}
	若对任意的$\lambda \in I$ (这里$I$取$[0, \infty)$或者$\mathbb{R}$)总有熵$\mathcal{H}(e^{\lambda X}) \leq \frac{\lambda^2 \sigma^2}{2} \cdot \E[e^{\lambda X}]$, 那么
	\begin{equation*}
...
\begin{remark}
	反之, 若$X$为参数是$\sigma/2$-的次高斯的, 那么有$\mathcal{H}(e^{\lambda X}) \leq \frac{\lambda^2 \sigma^2}{2} \cdot \E[e^{\lambda X}]$. 
	\begin{proof}
		考虑随机变量$Z := e^{\lambda X} / \E[e^{\lambda X}]$, 那么
		\begin{equation*}
			\E[Z \log Z] 
			= \frac{1}{\E[e^{\lambda X}]} \E \left[ e^{\lambda X} \left(\lambda X - \log \E[e^{\lambda X}] \right) \right]
			= \frac{\cH(e^{\lambda X})}{\E[e^{\lambda X}]}. 
		\end{equation*}
		考虑指数加权期望, 我们还有
		\begin{align*}
			\E[Z \log Z]
			= \frac{\E[\log Z \cdot e^{\lambda X}]}{\E[e^{\lambda X}]}
			= \E_{\lambda}[\log Z]
			\leq \log \E_{\lambda} [Z]
			= \log \E[e^{2\lambda X}] - 2 \log \E[e^{\lambda X}]
		\end{align*}
		由假设, 其中$\log \E[e^{2\lambda X}] \leq \lambda \E[X] + \frac{\lambda^2 \sigma^2}{2}$, 而$\log \E[e^{\lambda X}] \geq \lambda \E[X]$, 于是
		\begin{equation*}
			\mathcal{H}(e^{\lambda X})
			= \E[Z \log Z] \cdot \E[e^{\lambda X}]
			\leq \frac{\lambda^2 \sigma^2}{2} \cdot \E[e^{\lambda X}]. 
		\end{equation*}
	\end{proof}
\end{remark}
