...
\begin{proposition}[Hoeffding界]\label{thm:HoeffdingBd}
	设随机变量序列$\{X_i\}_{i=1}^n$中$X_i$为均值为$\mu_i$, 参数为$\sigma_i$的独立次高斯随机变量, 那么$\sum_i X_i$是$\sqrt{\sum_i \sigma_i^2}$-次高斯的, 于是我们有上偏差不等式
	\begin{equation*}
...
和次指数随机变量类似, 次指数随机变量也具有可加性, 只需相应的变换参数即可. 
\begin{proposition}
	设随机变量序列$\{X_i\}_{i=1}^n$中$X_i$为均值为$\mu_i$, 参数为$(\nu_i, \sigma_i)$的独立次指数随机变量, 那么$\sum_i X_i$是$(\nu_*, \alpha_*)$-次指数的, 其中
	\begin{equation*}
		\nu_* := \sqrt{\sum_{i=1}^n \nu_k^2}, \quad
		\alpha_* := \max_{1 \leq i \leq n} \alpha_i. 
	\end{equation*}
\end{proposition}


有时随机变量的矩生成函数无法得到, 我们可以通过控制随机变量的矩来来控制矩生成函数.   
称期望为$\mu$, 方差为$\sigma^2$的随机变量$X$满足参数为$b$的\textbf{Bernstein条件}, 如果
\begin{equation}\label{eq:BernsteinCondition}
	\left|\mathbb{E}\left[(X - \mu)^k\right]\right|
	\leq \frac{k!}{2} \sigma^2 b^{k-2}, \quad \forall k = 2,3,4, \dots
\end{equation}
例如当$X$绝对有界, 即$|X-\mu| \leq b$ a.s.时, 上述条件成立.
此外, 也有很多无界随机变量满足Bernstein条件, 并且Bernstein条件可能会得到比Hoeffding界更紧的尾部概率界, 这些都让Bernstein条件有着广泛的应用. 

当$X$满足Bernstein条件时, 对任意$|\lambda| < 1/b$, 我们有
\begin{align}\label{eq:MGFofBernsteinCondition}
	\mathbb{E}[e^{\lambda(X-\mu)}]
	& = 1 + \frac{\lambda^2 \sigma^2}{2} + \sum_{k=3}^{\infty} \frac{\mathbb{E}[(X-\mu)^k]}{k!} \lambda^k \notag \\ 
	&\leq 1 + \frac{\lambda^2 \sigma^2}{2} \sum_{k=0}^{\infty} (|\lambda| b)^k 
	= 1 + \frac{\lambda^2 \sigma^2}{2(1 - |\lambda| b)}
	\leq \exp \left( \frac{\lambda^2 \sigma^2}{2(1 - |\lambda| b)} \right). 
\end{align}
进一步地, 当$|\lambda| < 1/2b$时, $1 - |\lambda| b \geq 1/2$, 于是$\mathbb{E}[e^{\lambda(X-\mu)}] \leq e^{\frac{(\sqrt{2} \sigma)^2 \lambda^2}{2}}$, 即$X$为$(\sqrt{2} \sigma, 2b)$-次指数的. 
应用命题\ref{prop:UpperDeviationOfSubExpRV}可以得到满足Bernstein条件的随机变量的尾部概率界, 但是如果直接从不等式\eqref{eq:MGFofBernsteinCondition}出发, 在Chernoff不等式中取$\lambda = \frac{t^2}{bt + \sigma^2} < \frac{1}{b}$,至少可以得到常数项更紧的尾部概率界: 
\begin{proposition}[Bernstein界]
	若随机变量$X$满足Bernstein条件\eqref{eq:BernsteinCondition}, 那么
	\begin{equation}\label{eq:BernsteinBd}
		\P[|X-\mu| \geq t] \leq 2 e^{-\frac{t^2}{2(\sigma^2 + bt)}}, \quad \forall t \geq 0. 
	\end{equation}
\end{proposition}
\begin{remark}\label{rm:UsingDistributionMethod}
	对于有界随机变量$|X - \mu| \leq b$, 引理\ref{lemma:SubGaussianParameterOfBddRV} 说明了$X$是$b$-次高斯的. 
	而Bernstein界\eqref{eq:BernsteinBd}同时依赖于参数$\sigma$和$b$, 并且对于充分小的$t$, 随机变量的表现会和参数为$\sigma = \sqrt{\mathbb{E}[(X-\mu)^2]} \leq b$一致. 
	于是Bernstein界不会比原先的结果差——这里我们利用了方差的信息. 
	特别地, 对于方差小但是可能取到较大值的随机变量, 此时$\sigma \ll b$, 我们可以得到更紧的尾部概率界. 
\end{remark}

Bernstein条件成立的的一个充分条件是随机变量绝对有界, 然而如果只有单侧界时, 相应的我们可以得到单侧的结果. 
\begin{proposition}[单侧Bernstein不等式]
	...
	\end{equation*}
	相应的, 给定$n$个满足$X_i \leq b$ a.s. 的独立随机变量, 我们有
	\begin{equation*}
		\P \left[ \sum_{i=1}^n (X_i - \mathbb{E}[X_i]) \geq n \delta \right]
		\leq \exp \left(- \frac{n \delta^2}{2\left( \frac{1}{n} \sum_i \mathbb{E}[X_i^2] + \frac{b \delta}{3} \right)} \right). 
	\end{equation*}
\end{proposition}
\begin{proof}
	定义函数$h(u) := \frac{2}{u^2} (e^u - u - 1)$, 于是当$\lambda \geq 0$时, 中心化矩生成函数
	\begin{align*}
		\E[e^{\lambda (X - \E[X])}] 
		& = \left(1 + \lambda \mathbb{E}[X] + \frac{\lambda^2}{2} \mathbb{E}[X^2 h(\lambda X)]\right) e^{-\lambda \E[X]}
		\leq  \left( 1 + \lambda \mathbb{E}[X] + \frac{\lambda^2}{2} \mathbb{E}[X^2] h(\lambda b) \right) e^{-\lambda \E[X]} \\
		& \leq  \exp\left(\lambda \mathbb{E}[X] + \frac{\lambda^2}{2} \mathbb{E}[X^2] h(\lambda b)\right) \cdot e^{-\lambda \E[X]}
...
\begin{example}[点集的Rademacher复杂度]\label{ex:RCofPtSet}
	...
	\begin{equation*}
		...
		\leq 2 |a_k|, 
	\end{equation*}
	两边同时对$\bm{a}$在集合$A$上取上确界有$f(\bm{\epsilon}) - f(\bm{\epsilon}') \leq 2 \sup_{\bm{a} \in A} |a_k|$, 交换$\bm{\epsilon}$和$\bm{\epsilon}'$的位置可以得到相同的结论, 从而$f$是满足有界差不等式. 
	...
	\end{equation*}
	于是由定理\ref{thm:Azuma–Hoeffding}, $Z = f(\bm{\epsilon})$是$\sqrt{\sum_i \sup_{\bm{a} \in A} a_i^2}$-次高斯的. 
\end{example}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{熵方法}
...
\end{figure}
\noindent
从这一定理出发, 很容易证明经典等周不等式: 记$\B^n$为$\R^n$中的单位球, 对任意$A \subseteq \mathbb{R}^n$满足$\vol(A) = \vol(\B^n)$, 
\begin{align*}
